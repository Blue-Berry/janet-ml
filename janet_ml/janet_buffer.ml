module Make (I : Janet_sig.S) = struct
  ...
  let push_u16 (buf : t) (x : int) =
    F.janet_buffer_push_u16 buf (Unsigned.UInt16.of_int x)
  ;;

  let push_u32 (buf : t) (x : int) =
    F.janet_buffer_push_u32 buf (Unsigned.UInt32.of_int x)
  ;;

  let push_u64 (buf : t) (x : int) =
    F.janet_buffer_push_u64 buf (Unsigned.UInt64.of_int x)
  ;;
  ...
  let wrap (buf : t) : I.t = F.janet_wrap_buffer buf
  let unwrap (j : I.t) : t = F.janet_unwrap_buffer j
end
