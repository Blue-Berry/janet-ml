...
let set (builder : t) (i : int) (value : janet) =
...
let of_list (values : janet list) : t =
...
let of_array (values : janet array) : t = of_list (Array.to_list values)
...
let get (tup : t) (i : int) : janet =
  let d = data_of_head tup in
  Ctypes.(!@(d +@ i))
;;

let to_list (tup : t) : janet list =
  let n = length tup in
  Ctypes.CArray.from_ptr (data_of_head tup) n |> Ctypes.CArray.to_list
;;

let to_array (tup : t) : janet array = to_list tup |> Array.of_list

(* Wrap/unwrap convert between head pointer and Janet value *)
let wrap (tup : t) : janet = F.janet_wrap_tuple (data_of_head tup)
let unwrap (j : janet) : t = head_of_data (F.janet_unwrap_tuple j)
