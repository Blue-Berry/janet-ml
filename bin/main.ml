...
let () =
  with_janet_env (fun env ->
    ...
    let greet_fn =
      match
        Env.Binding.lookup ~env "greet" |> Env.Binding.to_janet |> Unwrapped.of_janet
      with
      | Unwrapped.Function f -> f
      | _ -> failwith "greet not found"
    in
    let greeting =
      Function.call_exn greet_fn [ Unwrapped.to_janet (Unwrapped.String "world") ]
    in
    ...
    in
    let ocaml_result =
      dostring_exn ~env {|(ocaml-add 10 32)|} ~source_path:(Some "main")
    in
